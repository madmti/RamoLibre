...
export abstract class DefaultStore<T> implements Writable<T> {
	...
	public static allInitialized(): boolean {
		return Array.from(DefaultStore.initializedInstances.values()).every(
	...
	set(value: T): void {
		if (typeof window !== 'undefined') {
			let storageValue =
				this.encrypted && this.encryptor
					? this.encryptor.encryptData(value)
	...
	update(updater: (value: T) => T): void {
		const currentValue = loadFromLocalStorage<T>(
			this.storageKey,
			this.defaultValue,
			this.encrypted
		);
