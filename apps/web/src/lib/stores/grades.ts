...
export class GradeManager extends DefaultStore<Grade[]> {
    ...
    getGradesCalculations(subjects: Subject[], method: AvailableMethods): Record<string, GradeCalculationResult> {
        const results: Record<string, GradeCalculationResult> = {};
        for (const subject of subjects) {
            results[subject.id] = gradeCalculator.calculate(
                subject,
                this,
                method,
            );
    ...
    getStats(gradeCalculations: Record<string, GradeCalculationResult>): Stats {
        ...
        const averageGrade = Object.values(gradeCalculations)
            .reduce((sum, result) => sum + result.currentGrade, 0) / totalSubjects || 0;

        const passingSubjects = Object.values(gradeCalculations)
            .filter(result => result.canPass).length;
